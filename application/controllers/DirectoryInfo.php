...
class DirectoryInfo extends APP_Controller
{
  ...
  public function index()
  {
    ...
    $this->load->view('user/user', array('first_name' => $first_name, 'mobile_no' => $mobile_no));
  }
  public function getAllDirectoryInfo()
  {
    $post = ($this->input->post());
    if ($this->input->is_ajax_request()) {
      $user_id = ($this->session->userdata('logged_session')['id']);
      $data['directoryInfo'] = $this->DirectoryInfo_model->getAllDirecetoryInfoData($user_id, $post);
  ...
  public function chartViewOfDirectory($id)
  {

    $id = ($this->uri->segment(3));
    // print_r($id);
    if (!empty($id)) {
      $directory_data = $this->DirectoryInfo_model->getDirectoryLogs($id);
      $shoe_date_wise_count = [];
      $i = 0;
      $data_db = array_column($directory_data, 'count', 'date');
      // print_r($data_db);
      while (true) {
        $start_date = date("Y-m-d", strtotime('-' . $i . 'days'));
        if (in_array($start_date, array_column($directory_data, 'date'))) {
          $shoe_date_wise_count[$start_date] = $data_db[$start_date];
        } else {
          $shoe_date_wise_count[$start_date] = 0;
        }
        $i++;
        if ($i == 7) {
          break;
      ...
      $data = [];
      foreach ($shoe_date_wise_count as $key => $value) {
        $dat_push = [];
        $dat_push['label'] = date("d/m/Y", strtotime($key));
        $dat_push['y'] = $value;
        array_push($data, $dat_push);
      }
      // echo "<pre>";
      // print_r($data);
      $this->load->view('user/user_directory_chart', array('data' => $data));
    }
  }
}
